...
#define GPIO_SEL_25 (1ULL << 25)  // Define manualmente o bitmask para GPIO 19
...
void setupWiFi() {
  ...
  bool res;
  res = wm.autoConnect("Apontados_ESP32","Instituto"); // password protected ap
...
void setup() {
  ...
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0) {
    Serial.println("Acordado do modo de suspensão");
    lcd.setCursor(0, 0);
    lcd.print("Acordando...");
    delay(2000);
  } else {
...
void loop() {
  desligarEsp32();   // Verifica se o botão foi pressionado para desligar o ESP32
  reiniciarEsp32(); // Verifica se o outro botão foi pressionado para reiniciar o ESP32
...
void configurarPinos() {
  ...
  pinMode(BUTTON, INPUT_PULLUP);  // Configurando botão como INPUT_PULLUP
  pinMode(BUTTON2, INPUT_PULLUP);  // Configurando botão como INPUT_PULLUP
...
void reiniciarEsp32() {
  ...
  if (leituraR == LOW && millis() - ultimoTempoBotao > intervaloDebounce) {
    ...
    // Desabilite outras fontes de wake-up
    esp_sleep_disable_wakeup_source(ESP_SLEEP_WAKEUP_EXT1);

    // Configure apenas o temporizador como fonte de wake-up
    esp_sleep_enable_timer_wakeup(10 * 500000); // Acordar após 5 segundos
